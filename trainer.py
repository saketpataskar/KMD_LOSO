...
def train_one_epoch(model, optim, loss_fn, dataloader, device, scaler):
    model.train()
    total_loss = 0.0
...
def run_train_validation(model, train_ds, val_ds, device, out_dir,
                         epochs=50, batch_size=128, lr=1e-3, patience=8):
    ...
    for epoch in range(1, epochs+1):
        t0 = time.time()
        tr_loss, tr_acc, tr_f1 = train_one_epoch(model, optim, loss_fn, train_loader, device, scaler)
